...
def forward(B, U, dim,
    ...
    def main(
            ...
            Z: T.Tensor((B, dim, U), dtype), # type: ignore
    ):
        # T.func_attr({"tir.noalias": True})
        # T.func_attr({"tl.copy_vectorize": 4}) 

        with T.Kernel(B, T.ceildiv(U, block_U), threads=threads) as (b_idx, u_idx):

            X_shared = T.alloc_shared((block_U,), dtype)
            T.copy(B_buf[b_idx, u_idx * block_U], X_shared)

            Y_shared = T.alloc_shared((dim,), dtype)
            T.copy(Y[b_idx, :], Y_shared)

            Z_local = T.alloc_fragment((dim, block_U), dtype)
            T.clear(Z_local)

            T.sync_threads()

            for i, j in T.Parallel(dim, block_U):
                Z_local[i, j] = X_shared[j] * Y_shared[i]

            T.copy(Z_local, Z[b_idx, 0, u_idx * block_U])
...
for retry_id in range(0, retry):
    ...
    for dim_idx in range(0, path_num):
        dim = dim_list[dim_idx]
        kernel = forward(B, U, dim)

        y = Y[:, dim_offset:dim_offset + dim].contiguous()
        B_buf = b_buf[:, dim_idx, :].contiguous()
        # print(B_buf.shape)

        out = torch.zeros(B, dim, U, dtype=dtype, device=device)
...
tilelang_err = (out_cueq - out_tilelang).abs()
print(f"tilelang_err: {tilelang_err.max().item()}")
